...
void EchoServer::handleClient(sylar::Socket::ptr client) {
    __LOG_INFO(g_logger) << "handleClient " << *client;   
    ...
    while(true) {
        ...
        if(rt == 0) {
            __LOG_INFO(g_logger) << "client close: " << *client;
            break;
        } else if(rt < 0) {
            __LOG_INFO(g_logger) << "client error rt=" << rt
...
void run() {
    __LOG_INFO(g_logger) << "server type=" << type;
...
int main(int argc, char** argv) {
    if(argc < 2) {
        __LOG_INFO(g_logger) << "used as[" << argv[0] << " -t] or [" << argv[0] << " -b]";
        return 0;
    }
